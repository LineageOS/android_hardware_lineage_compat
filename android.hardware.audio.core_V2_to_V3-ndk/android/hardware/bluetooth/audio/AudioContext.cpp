...
namespace bluetooth {
namespace audio {
const char* AudioContext::descriptor = "android.hardware.bluetooth.audio.AudioContext";

binder_status_t AudioContext::readFromParcel(const AParcel* _aidl_parcel) {
  ...
  _aidl_ret_status = ::ndk::AParcel_readData(_aidl_parcel, &bitmask);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  AParcel_setDataPosition(_aidl_parcel, _aidl_start_pos + _aidl_parcelable_size);
  return _aidl_ret_status;
}
binder_status_t AudioContext::writeToParcel(AParcel* _aidl_parcel) const {
