...
public:
  ...
  static constexpr uint32_t TRANSACTION_onTransferReady = FIRST_CALL_TRANSACTION + 0;
  static constexpr uint32_t TRANSACTION_onError = FIRST_CALL_TRANSACTION + 1;
  static constexpr uint32_t TRANSACTION_onDrainReady = FIRST_CALL_TRANSACTION + 2;

  static std::shared_ptr<IStreamCallback> fromBinder(const ::ndk::SpAIBinder& binder);
  static binder_status_t writeToParcel(AParcel* parcel, const std::shared_ptr<IStreamCallback>& instance);
  static binder_status_t readFromParcel(const AParcel* parcel, std::shared_ptr<IStreamCallback>* instance);
  static bool setDefaultImpl(const std::shared_ptr<IStreamCallback>& impl);
  static const std::shared_ptr<IStreamCallback>& getDefaultImpl();
  virtual ::ndk::ScopedAStatus onTransferReady() = 0;
  virtual ::ndk::ScopedAStatus onError() = 0;
  virtual ::ndk::ScopedAStatus onDrainReady() = 0;
