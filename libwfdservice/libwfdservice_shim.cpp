...
android::media::AudioPortFw aidlPort() {
    ...
    const auto devDescr =
            android::sp<android::DeviceDescriptorBase>::make(AUDIO_DEVICE_OUT_PROXY, "");
...
extern "C" int _ZN18WiFiDisplaySession31broadcastWifiDisplayAudioIntentEb(
    ...
    if (enabled) {
        ...
        android::AudioSystem::setDeviceConnectionState(
                audio_policy_dev_state_t::AUDIO_POLICY_DEVICE_STATE_AVAILABLE, aidlPort().hal,
                audio_format_t::AUDIO_FORMAT_PCM);
    } else {
        thisptr->c = false;

        android::AudioSystem::setDeviceConnectionState(
                audio_policy_dev_state_t::AUDIO_POLICY_DEVICE_STATE_UNAVAILABLE, aidlPort().hal,
                audio_format_t::AUDIO_FORMAT_PCM);
    }
